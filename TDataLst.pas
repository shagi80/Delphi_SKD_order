...
var
  i   : integer;
begin
  for I := 0 to FCount - 1 do self.FModels[i].Free;
  inherited Destroy;
end;
...
procedure TModList.SaveAll(Fname: string);
var
  i,j,k        : integer;
  str          : string;
  Strs         : TStringList;
...
begin
  ...
  Strs.Add(IntToStr(self.Count));
  for I := 0 to self.Count - 1 do
    begin
      Strs.Add(self.Model[i].EngName);
      Strs.Add(IntToStr(self.Model[i].Count));
      for j := 0 to self.Model[i].Count - 1 do
        begin
          ModItm:=self.Model[i].Item[j];
